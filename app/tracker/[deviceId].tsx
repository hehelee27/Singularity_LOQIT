...
function InteractiveMap({ latitude, longitude, mapRef, pulse, path = [], colors, isDark }: any) {
  ...
  return (
    ...
    >
      ...
      {path.map((p, i) => <Circle key={i} center={p} radius={i === 0 ? 0 : 5} fillColor={i === 0 ? 'transparent' : `${colors.primary}80`} strokeWidth={0} />)}
      <Marker coordinate={{ latitude, longitude }}>
        <View style={styles.markerWrap}>
          <Animated.View style={[styles.markerPulse, { borderColor: `${colors.primary}80`, backgroundColor: `${colors.primary}30`, opacity: pulse.interpolate({ inputRange: [0, 1], outputRange: [0.46, 0] }), transform: [{ scale: pulse.interpolate({ inputRange: [0, 1], outputRange: [0.65, 1.42] }) }] }]} />
          <View style={[styles.markerCore, { backgroundColor: colors.surfaceContainerHighest, borderColor: colors.primary }]}>
...
export default function TrackerScreen() {
  ...
  const fetchTrackerData = useCallback(async () => {
    ...
    const [{ data: d }, { data: l }] = await Promise.all([
      supabase.from('devices').select('id, make, model').eq('id', deviceId).maybeSingle(),
      supabase.from('beacon_logs').select('*').eq('device_id', deviceId).order('reported_at', { ascending: false }).limit(20)
    ])
    setDevice(d as any); setLogs(l as any); setLoading(false)
  }, [deviceId, user])
