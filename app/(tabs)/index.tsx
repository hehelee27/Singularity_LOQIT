...
export default function HomeScreen() {
  ...
  const quickActions = useMemo(() => [
    { key: 'sync', icon: 'location-searching' as const, label: 'Sync Location', tint: colors.primary },
    ...
    { key: 'alerts', icon: 'notifications-active' as const, label: 'My Alerts', tint: colors.tertiary, route: '/(tabs)/alerts' },
  ], [colors])
  ...
  useEffect(() => {
    ...
    const handleAppStateChange = (nextState: typeof AppState.currentState) => {
      if (appStateRef.current.match(/inactive|background/) && nextState === 'active') { runPassiveScan() }
      appStateRef.current = nextState
    }
    const appStateSubscription = AppState.addEventListener('change', handleAppStateChange)
    runPassiveScan()
    passiveScanRef.current = setInterval(() => { runPassiveScan() }, 30000)
    return () => {
  ...
  const syncLocation = async () => {
    ...
    try {
      ...
      await supabase.from('devices').update({ last_seen_lat: loc.coords.latitude, last_seen_lng: loc.coords.longitude, last_seen_at: new Date().toISOString() }).eq('owner_id', user.id)
      await refetch()
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {loadingDevices ? (
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 12 }}>
            {devices.map(d => (
              <DeviceCard key={d.id} {...d} imei={d.imei_primary} onPress={id => router.push({ pathname: '/device/[id]', params: { id } })} />
            ))}
            {!devices.length && (
        ...
        <View style={styles.quickGrid}>
          {quickActions.map(a => (
            <Pressable key={a.key} style={[styles.quickCard, { backgroundColor: colors.surfaceContainerLow, borderColor: colors.outlineVariant }]} onPress={() => { if (a.key === 'sync') syncLocation(); else if (a.route) router.push(a.route); else setShowReportPicker(true); }}>
        ...
        <View style={styles.modalBackdrop}>
          <View style={[styles.sheet, { backgroundColor: colors.surfaceContainer }]}>
            ...
            <View style={{ gap: 8, marginTop: 16 }}>
              {devices.map(d => (
                <Pressable key={d.id} style={styles.sheetRow} onPress={() => { setShowReportPicker(false); router.push({ pathname: '/device/[id]', params: { id: d.id } }) }}>
...
const styles = StyleSheet.create({
  ...
  sheetCancelText: { fontFamily: FontFamily.bodySemiBold, fontSize: 16 }
})